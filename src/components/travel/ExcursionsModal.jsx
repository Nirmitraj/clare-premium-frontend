// /src/components/travel/ExcursionsModal.jsx
import React, { useEffect, useMemo, useRef, useState } from "react";
import { createPortal } from "react-dom";
import { X } from "lucide-react";

const ACTIVITY_TYPES = [
  "Cultural","Historic","Culinary","Extreme","Ecotourism",
  "Landmarks","Sports","Adventure","Heritage","Mystical"
];

const STYLE_OPTIONS = [
  "Guided Tours","Aerial Tours","Full Day Trips","Tickets",
  "Multi Day Tours","Gastronomic Tours","Sightseeing Tours","Other"
];
...
export default function ExcursionsModal({
  ...
  onSubmit,
  title = "Excursions",
  subtitle = "Custom day trips, guided tours & special outings",
  imageUrl = "/media/excursions.png",
  info = [
    "Handcrafted excursions around landmarks and hidden gems.",
    "Private guides or small groups, tailored to your pace.",
    "Logistics handled end-to-end — just show up and enjoy."
  ],
}) {
  ...
  const totalPeople =
    Number(form.children) + Number(form.adults) + Number(form.seniors) + Number(form.students);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        activitiesName: form.activitiesName.trim(),
        startDate: form.startDate,
        endDate: form.endDate,
        activityTypes: form.activityTypes, // array
        ...
        stylesOther: form.styles.includes("Other") ? form.stylesOther : "",
        photoPackage: form.photoPackage,
        privateToursInfo: form.privateToursInfo,
  ...
  return createPortal(
    ...
    >
      {/* Local styles: shimmer/shine + gold helpers */}
      <style>{`
      ...
      {/* Panel with gold gradient border */}
      <div
        className={[
          "relative z-[101]",
        ...
        <div className="relative gold-card h-full rounded-none md:rounded-2xl">
          ...
          <div className="sticky top-0 z-[2] flex justify-end p-3 md:p-0 md:block md:static">
            <button
              onClick={onClose}
              disabled={submitting}
              className="inline-flex items-center justify-center rounded-lg p-2 md:absolute md:right-3 md:top-3 hover:bg_WHITE/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#d4af37]/70 disabled:opacity-60"
              aria-label="Close"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="relative h-[36vh] md:h-auto">
              <div className="absolute inset-0">
                <img src={imageUrl} alt="Excursions" className="h-full w-full object-cover opacity-70" />
              ...
              {done ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-white/80 mt-1">
                    Your excursions enquiry has been submitted. We’ll be in touch soon.
                  </p>
                  <button
                    onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
                  ...
                  <div>
                    <label className="block text-sm text-white/85 mb-1">Activities Name</label>
                    <input
                      type="text"
                      placeholder="Enter Activities Name"
                      className="w-full rounded-lg bg-white/5 border border-[#b69333]/30 px-3 py-2 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-[#ffd369]/60"
                      value={form.activitiesName}
                      onChange={update("activitiesName")}
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm text-white/85 mb-1">
                        Start Date<span className="text-[#ffd369]">*</span>
                      </label>
                      <input
                        type="date"
                    ...
                    <div>
                      <label className="block text-sm text-white/85 mb-1">
                        End Date<span className="text-[#ffd369]">*</span>
                      </label>
                      <input
                        type="date"
                        className="w-full rounded-lg bg-white/5 border border-[#b69333]/30 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#ffd369]/60"
                  ...
                  <div>
                    <label className="block text-sm text-white/85 mb-2">Activities Type (select any)</label>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm text-white">
                      {ACTIVITY_TYPES.map((opt) => (
                        <label
                          key={opt}
                          className={`flex items-center gap-2 rounded-lg border px-3 py-2 cursor-pointer transition ${
                            form.activityTypes.includes(opt)
                        ...
                        >
                          <input
                            ...
                            checked={form.activityTypes.includes(opt)}
                            onChange={toggleIn("activityTypes", opt)}
                          />
                          {opt}
                        </label>
                      ))}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <NumberField label="Seniors (65–99)" value={form.seniors} onChange={updateNum("seniors", 0)} />
                    <NumberField label="Students (13–22)" value={form.students} onChange={updateNum("students", 0)} />
                  </div>
                  ...
                  {/* Style options (multi) */}
                  <div>
                    <label className="block text-sm text-white/85 mb-2">Select the option that fits your style</label>
                    <div className="grid grid-cols-2 sm:grid-cols-2 gap-2 text-sm text-white">
                      {STYLE_OPTIONS.map((opt) => (
                        <label
                          key={opt}
                          className={`flex items-center gap-2 rounded-lg border px-3 py-2 cursor-pointer transition ${
                            form.styles.includes(opt)
                              ? "border-[#d4af37] bg-[#d4af37]/10"
                              : "border-[#b69333]/30 bg-white/5 hover:bg-white/10"
                          }`}
                        >
                          <input
                            type="checkbox"
                            className="accent-[#d4af37]"
                            checked={form.styles.includes(opt)}
                            onChange={toggleIn("styles", opt)}
                          />
                          {opt}
                        </label>
                      ))}
                    ...
                    {form.styles.includes("Other") && (
                      <input
                        ...
                        placeholder="Tell us more"
                        className="mt-2 w-full rounded-lg bg-white/5 border border-[#b69333]/30 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#ffd369]/60"
                        value={form.stylesOther}
                        onChange={update("stylesOther")}
                      />
                    )}
                  </div>

                  {/* Toggles */}
                  <YesNo label="Photo Package" value={form.photoPackage} onChange={update("photoPackage")} />
                  <YesNo label="Information on Private Tours" value={form.privateToursInfo} onChange={update("privateToursInfo")} />
                  ...
                  <div>
                    ...
                    <textarea
                      maxLength={1000}
                      placeholder="Preferred route, pace, accessibility needs, dietary notes, etc."
