// /src/components/travel/AirlineTicketsModal.jsx
import React, { useEffect, useMemo, useRef, useState } from "react";
import { createPortal } from "react-dom";
import { X } from "lucide-react";

export default function AirlineTicketsModal({
  open,
  onClose,
  onSubmit,
  title = "Airline Tickets",
  subtitle = "Best routes, sharp fares, priority support",
  imageUrl = "/media/airlinetickets.png",
  info = [
    "Handpicked itineraries with ideal layovers & change protections.",
    "Access to premium cabins, mileage upgrades & lounge options.",
    "Seamless extras: assistance, transfers, meals, seat selection."
  ],
}) {
  ...
  return createPortal(
    ...
    >
      {/* Local styles: same gold/shimmer system as Hotels */}
      <style>{`
      ...
      >
        ...
        <div className="relative gold-card h-full rounded-none md:rounded-2xl">
          ...
          <div className="sticky top-0 z-[2] flex justify-end p-3 md:p-0 md:block md:static">
            <button
              onClick={onClose}
              className="inline-flex items-center justify-center rounded-lg p-2 md:absolute md:right-3 md:top-3 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#d4af37]/70"
              aria-label="Close"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="relative h-[36vh] md:h-auto">
              <div className="absolute inset-0">
                <img src={imageUrl} alt="Airline Tickets" className="h-full w-full object-cover opacity-70" />
              ...
              {done ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-white/80 mt-1">
                    Your flight enquiry has been submitted. Our team will reach out shortly.
                  </p>
                  <button
                    onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm text-white/85 mb-1">
                        From<span className="text-[#ffd369]">*</span>
                      </label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-sm text-white/85 mb-1">
                        To<span className="text-[#ffd369]">*</span>
                      </label>
                      <input
                        type="text"
                        placeholder="City or Airport"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm text-white/85 mb-1">
                        Departure<span className="text-[#ffd369]">*</span>
                      </label>
                      <input
                        type="date"
                    ...
                    <div>
                      <label className="block text-sm text-white/85 mb-1">
                        Return{needReturn && <span className="text-[#ffd369]">*</span>}
                      </label>
                      <input
                        type="date"
                        className="w-full rounded-lg bg-white/5 border border-[#b69333]/30 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#ffd369]/60 disabled:opacity-50"
                  ...
                  {/* Class */}
                  <div>
                    <label className="block text-sm text-white/85 mb-1">Class</label>
                    <select
                      className="w-full rounded-lg bg-white/5 border border-[#b69333]/30 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#ffd369]/60"
                  ...
                  {/* Yes/No toggles */}
                  <YesNo label="Transportation to the Hotel" value={form.transportToHotel} onChange={update("transportToHotel")} />
                  <YesNo label="Wheel Chair Assistance" value={form.wheelchair} onChange={update("wheelchair")} />

                  {/* Special meal + FFP */}
                  <div>
                    <label className="block text-sm text-white/85 mb-1">Special Meal Requests</label>
                    <input
                      type="text"
                      maxLength={100}
                      placeholder="e.g., Vegetarian, Kosher, Diabetic"
                      className="w-full rounded-lg bg-white/5 border border-[#b69333]/30 px-3 py-2 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-[#ffd369]/60"
                      value={form.mealRequest}
                      onChange={update("mealRequest")}
                    />
                    <div className="text-right text-xs text-white/60">{form.mealRequest.length}/100</div>
                  </div>

                  <div>
                    <label className="block text-sm text-white/85 mb-1">Frequent Flyer Information</label>
                    <input
                      type="text"
                      placeholder="Airline / Membership Number"
                      className="w-full rounded-lg bg-white/5 border border-[#b69333]/30 px-3 py-2 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-[#ffd369]/60"
                      value={form.frequentFlyer}
                      onChange={update("frequentFlyer")}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      maxLength={1000}
                      placeholder="Seat preferences, schedule constraints, connection notes, etc."
