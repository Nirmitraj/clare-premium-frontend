// /src/components/concierge/GiftDeliveryModal.jsx
...
export default function GiftDeliveryModal({
  open,
  onClose,
  onSubmit,
  title = "World Wide Gift Delivery",
  subtitle = "Same-day & international gifting, curated by your Concierge",
  imageUrl = "/media/gift-delivery.png",
  info = [
    "From classic flowers to bespoke hampers and vintages.",
    "Door-to-door delivery with precise timing and messaging.",
    "Perfect for celebrations, romance, or just-because gestures.",
  ],
}) {
  const [form, setForm] = useState({
    ...
    address: "", // Street - Number - City - State - Country - Zip Code
    phoneNumber: "",
    phoneNumber2: "",
    email: "",
  ...
  const updateNum = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value.replace(/[^\d+()\-\s]/g, "") }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        giftIdeaOther: form.giftIdea === "Others" ? form.giftIdeaOther : "",
        occasion: form.occasion,
        occasionOther: form.occasion === "Other" ? form.occasionOther : "",
        ...
        deliveryTime: form.deliveryTime, // optional
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="relative h-[36vh] md:h-auto">
              <div className="absolute inset-0">
                <img src={imageUrl} alt="World Wide Gift Delivery" className="h-full w-full object-cover opacity-70" />
            ...
            <div className="p-5 sm:p-6 md:p-7 overflow-y-auto max-h-[54vh] md:max-h-[90vh]">
              {done ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-white/80 mt-1">Your gift delivery request has been submitted. We’ll be in touch soon.</p>
                  <button
                    onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
                  ...
                  <div>
                    <label className="block text-sm text-white/85 mb-1">
                      My Gift Idea<span className="text-[#ffd369]">*</span>
                    </label>
                      ...
                      {GIFT_IDEAS.map((opt) => (
                        ...
                        >
                          <input
                            ...
                            checked={form.giftIdea === opt}
                            onChange={() => setForm((f) => ({ ...f, giftIdea: opt }))}
                            required
                          />
                          {opt}
                        </label>
                    ...
                    {form.giftIdea === "Others" && (
                      <input
                        ...
                        value={form.giftIdeaOther}
                        onChange={update("giftIdeaOther")}
                      />
                    )}
                  </div>
                  ...
                  {/* Occasion */}
                  <div>
                    <label className="block text-sm text-white/85 mb-1">Select Occasion</label>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-white">
                      {OCCASIONS.map((opt) => (
                        <label
                          key={opt}
                        ...
                        >
                          <input
                            ...
                            name="occasion"
                            className="accent-[#d4af37]"
                            checked={form.occasion === opt}
                            onChange={() => setForm((f) => ({ ...f, occasion: opt }))}
                          />
                          {opt}
                        </label>
                      ))}
                    ...
                    {form.occasion === "Other" && (
                      <input
                        ...
                        onChange={update("occasionOther")}
                      />
                    )}
                  </div>

                  {/* Special Requests + Plan */}
                  <div>
                    <label className="block text-sm text-white/85 mb-1">Special Requests (max 1000 chars)</label>
                    <textarea
                      maxLength={1000}
                      placeholder="Delivery notes, flavor/allergy constraints, brand preferences, budget, etc."
                  ...
                  <div>
                    <label className="block text-sm text-white/85 mb-1">
                      Delivery to (Address)<span className="text-[#ffd369]">*</span>
                    </label>
                    <input
                      type="text"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm text-white/85 mb-1">
                        Phone Number<span className="text-[#ffd369]">*</span>
                      </label>
                      <input
                        type="tel"
                        placeholder="Area Code + 10 Digits"
                        className="w-full rounded-lg bg-white/5 border border-[#b69333]/30 px-3 py-2 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-[#ffd369]/60"
                        value={form.phoneNumber}
                        onChange={updateNum("phoneNumber")}
                        required
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={updateNum("phoneNumber2")}
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm text-white/85 mb-1">
                        Delivery Date<span className="text-[#ffd369]">*</span>
                      </label>
                      <input
                        type="date"
                  ...
                  {/* Gift message */}
                  <div>
                    <label className="block text-sm text-white/85 mb-1">Gift Message</label>
                    <textarea
                      placeholder="Your message on the card"
                      className="w-full min-h-[80px] rounded-lg bg-white/5 border border-[#b69333]/30 px-3 py-2 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-[#ffd369]/60"
                      value={form.giftMessage}
                      onChange={update("giftMessage")}
                    />
                  </div>
