// /src/components/lifestyle/WellnessEnquiryModal.jsx
import React, { useEffect, useMemo, useRef, useState } from "react";
import { createPortal } from "react-dom";
import { X } from "lucide-react";

const TREATMENTS = [
  "Massages","Facial","Grooming","Body Scrubs and Wraps",
  "Bride","Men's Retreat","Other"
];

const THERAPIST_GENDER = ["Male", "Female", "No preference"];

const SALON_SERVICES = [
  "Manicure","Makeup","Pedicure","Barber Services","Hair","Special Hair Services","Hair Enhancements"
];

const FITNESS_WELLNESS = [
  "Exercise-Movement","Sports Masssage","Relaxing Massage","Water Aerobics",
  "Exercise Physiology","Holistic Massage","Yoga classes"
];

const IN_SUITE = [
  "Makeup","Barber Services","Massage Services","Manicure","Pedicure","Facial Treatments"
];

export default function WellnessEnquiryModal({
  open,
  onClose,
  onSubmit,
  title = "Wellness Enquiry",
  subtitle = "Spa, salon, fitness & in-suite services",
  imageUrl = "/media/wellness.png",
  info = [
    "Massages, facials, wraps, and curated wellness.",
    "Salon, fitness classes, and in-suite services.",
    "Discreet scheduling with your preferences honored."
  ],
}) {
  ...
  const valid =
    ...
    form.date &&
    !!form.treatment &&
    (!!form.time || true); // time is optional, keep UX flexible
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        date: form.date,
        time: form.time, // optional

        treatment: form.treatment,
        treatmentOther: form.treatment === "Other" ? form.treatmentOther : "",
        ...
        salonServices: form.salonServices,
        fitnessWellness: form.fitnessWellness,
        inSuiteServices: form.inSuiteServices,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="relative h-[36vh] md:h-auto">
              <div className="absolute inset-0">
                <img src={imageUrl} alt="Wellness" className="h-full w-full object-cover opacity-70" />
            ...
            <div className="p-5 sm:p-6 md:p-7 overflow-y-auto max-h-[54vh] md:max-h-[90vh]">
              {done ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-white/80 mt-1">Your wellness enquiry has been submitted. We’ll be in touch soon.</p>
                  <button
                    onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="block text-sm text-white/85 mb-1">Time</label>
                      <input
                        type="time"
                        className="w-full rounded-lg bg-white/5 border border-[#b69333]/30 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#ffd369]/60"
                        value={form.time}
                  ...
                  <div>
                    <label className="block text-sm text-white/85 mb-1">
                      Treatment<span className="text-[#ffd369]">*</span>
                    </label>
                      ...
                      {TREATMENTS.map((opt) => (
                        ...
                        >
                          <input
                            ...
                            checked={form.treatment === opt}
                            onChange={() => setForm((f) => ({ ...f, treatment: opt }))}
                            required
                          />
                          {opt}
                        </label>
                    ...
                    {form.treatment === "Other" && (
                      <input
                        type="text"
                        placeholder="Tell us more about your treatment"
                        ...
                        onChange={update("treatmentOther")}
                      />
                    )}
                  </div>
                  ...
                  {/* Therapist gender */}
                  <div>
                    <label className="block text-sm text-white/85 mb-1">Therapist Gender</label>
                    <div className="grid grid-cols-3 gap-2 text-sm text-white">
                      {THERAPIST_GENDER.map((opt) => (
                        <label
                          key={opt}
                          className={`flex items-center gap-2 rounded-lg border px-3 py-2 cursor-pointer transition ${
                            form.therapistGender === opt
                              ? "border-[#d4af37] bg-[#d4af37]/10"
                              : "border-[#b69333]/30 bg-white/5 hover:bg-white/10"
                          }`}
                        >
                          <input
                            ...
                            className="accent-[#d4af37]"
                            checked={form.therapistGender === opt}
                            onChange={() => setForm((f) => ({ ...f, therapistGender: opt }))}
                          />
                          {opt}
                        </label>
                      ))}
                  ...
                  <CheckboxGroup
                    ...
                    options={SALON_SERVICES}
                    selected={form.salonServices}
                    onToggle={(opt) => toggleIn("salonServices", opt)()}
                  />

                  {/* Fitness & Wellness */}
                  <CheckboxGroup
                    label="Fitness & Wellness"
                    options={FITNESS_WELLNESS}
                    selected={form.fitnessWellness}
                    onToggle={(opt) => toggleIn("fitnessWellness", opt)()}
                  />

                  {/* In-Suite Services */}
                  <CheckboxGroup
                    label="In-Suite Services"
                    options={IN_SUITE}
                    selected={form.inSuiteServices}
                    onToggle={(opt) => toggleIn("inSuiteServices", opt)()}
                  />
                  ...
                  <div>
                    ...
                    <textarea
                      maxLength={1000}
                      placeholder="Allergies, preferences, accessibility, setting (in-suite vs spa), etc."
...
function CheckboxGroup({ label, options, selected, onToggle }) {
  return (
    <div>
      <label className="block text-sm text-white/85 mb-2">{label}</label>
